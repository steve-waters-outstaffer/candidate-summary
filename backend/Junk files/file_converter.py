...
def convert_to_supported_format(file_bytes: bytes, original_filename: str) -> tuple[bytes, str]:
    ...
    """
    # Use python-magic to get a more reliable MIME type from the file's content
    ...
    raise UnsupportedFileTypeError(
        f"File '{original_filename}' with MIME type '{detected_mime_type}' is not supported for upload."
    )