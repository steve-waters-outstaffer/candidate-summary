...
# OPTIMIZATION: Use a global session for connection pooling
# This enables HTTP keep-alive and reduces TCP/TLS handshake latency for subsequent requests.
...
def fetch_recruitcrm_candidate(slug):
    ...
    try:
        response = _session.get(url, headers=get_recruitcrm_headers())
...
def fetch_candidate_interview_id(candidate_slug, job_slug=None, candidate_data=None, job_specific_fields=None):
    ...
    Accepts optional pre-fetched candidate_data and job_specific_fields to avoid redundant API calls.
    """
    ...
    if job_slug:
        ...
        if job_specific_fields:
            # Handle both list (bulk/multi) and dict (single) formats of job_specific_fields
    ...
    # 2. Check general candidate fields (pre-fetched or fetch now)
...
def fetch_recruitcrm_job(slug, include_custom_fields=True):
    ...
    try:
        response = _session.get(url, headers=get_recruitcrm_headers(), params=params)
        response.raise_for_status()
...
def push_to_recruitcrm_internal(candidate_slug, html_summary):
    ...
    try:
        ...
        response = _session.post(url, files=files, headers=get_recruitcrm_headers())
...
def fetch_alpharun_interview(job_opening_id, interview_id):
    ...
    try:
        response = _session.get(url, headers=get_alpharun_headers())
        response.raise_for_status()
...
def create_recruitcrm_note(candidate_slug, job_slug, note_content):
    ...
    try:
        response = _session.post(url, headers=get_recruitcrm_headers(), json=payload)
        response.raise_for_status()
