...
def fetch_candidate_interview_id(candidate_slug, job_slug=None, candidate_data=None, job_specific_fields=None):
    ...
    Accepts optional candidate_data and job_specific_fields to avoid redundant API calls.
    """
    log.info("recruitcrm.fetch_candidate_interview_id.called", candidate_slug=candidate_slug, job_slug=job_slug)
    if job_slug:
        # Use provided fields if available, otherwise fetch them
        fields = job_specific_fields if job_specific_fields is not None else fetch_recruitcrm_candidate_job_specific_fields(candidate_slug, job_slug)
        if fields:
    ...
    data = candidate_data if candidate_data is not None else fetch_recruitcrm_candidate(candidate_slug)
    if data:
