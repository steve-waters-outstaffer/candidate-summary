...
def fetch_recruitcrm_candidate(slug):
    ...
    try:
        response = api_session.get(url, headers=get_recruitcrm_headers())
...
def fetch_candidate_interview_id(candidate_slug, job_slug=None, candidate_data=None, job_specific_fields=None):
    """Fetches the AI Interview ID for a candidate, checking job-specific fields first. Uses pre-fetched data if provided."""
    log.info("recruitcrm.fetch_candidate_interview_id.called", candidate_slug=candidate_slug, job_slug=job_slug)

    # 1. Check job-specific fields (pre-fetched or fetch now)
    if job_slug:
        ...
        if job_specific_fields:
            # handle both list and dict formats if necessary, but recruitcrm_helpers usually returns dict
            fields_to_check = job_specific_fields.values() if isinstance(job_specific_fields, dict) else job_specific_fields
            for field_data in fields_to_check:
    ...
    # 2. Check candidate general custom fields (pre-fetched or fetch now)
    if candidate_data is None:
        candidate_data = fetch_recruitcrm_candidate(candidate_slug)

    if candidate_data:
        # candidate_data might be the full response or just the 'data' part
        details = candidate_data.get('data', candidate_data)
...
def fetch_recruitcrm_job(slug, include_custom_fields=True):
    ...
    try:
        response = api_session.get(url, headers=get_recruitcrm_headers(), params=params)
        response.raise_for_status()
...
def push_to_recruitcrm_internal(candidate_slug, html_summary):
    ...
    try:
        ...
        response = api_session.post(url, files=files, headers=get_recruitcrm_headers())
...
def fetch_alpharun_interview(job_opening_id, interview_id):
    ...
    try:
        response = api_session.get(url, headers=get_alpharun_headers())
        response.raise_for_status()
...
def create_recruitcrm_note(candidate_slug, job_slug, note_content):
    ...
    try:
        response = api_session.post(url, headers=get_recruitcrm_headers(), json=payload)
        response.raise_for_status()
