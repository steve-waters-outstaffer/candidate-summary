...
def test_quil():
    ...
    try:
        ...
        return jsonify({'error': 'An internal error occurred during Quil testing'}), 500
...
def push_to_recruitcrm():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            log.error("single.push_to_recruitcrm.failed", status=response.status_code, response_text=response.text)
...
def move_stage():
    ...
    try:
        ...
        return jsonify({'error': 'An internal error occurred while moving candidate stage'}), 500
